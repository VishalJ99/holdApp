//
//  LeafSelectorPanel.swift
//  HoldApp
//
//  Floating panel for leaf selector (Cmd+Shift+S)
//  Minimal, terminal-inspired aesthetic matching SpotlightPanel
//

import Cocoa

class LeafSelectorPanel: NSPanel {
    ...
    /// Show the panel with leaf list
    func show(leaves: [(id: String, text: String)], currentIndex: Int) {
        print("🎯 [Panel.show] Called with \(leaves.count) leaves, currentIndex: \(currentIndex)")

        // Adjust height based on leaf count
        let rowHeight: CGFloat = 36  // 32px row + 4px spacing
        let minHeight: CGFloat = 100
        let maxHeight: CGFloat = 500
        let calculatedHeight = CGFloat(leaves.count) * rowHeight + 40  // +40 for padding
        ...
        // Update view controller with leaves
        print("🎯 [Panel.show] contentViewController: \(self.contentViewController != nil ? "exists" : "nil")")
        if let viewController = self.contentViewController as? LeafSelectorViewController {
            print("🎯 [Panel.show] Casting to LeafSelectorViewController succeeded")
            viewController.updateLeaves(leaves, currentIndex: currentIndex)
        } else {
